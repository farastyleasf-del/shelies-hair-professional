/**
 * WhatsApp DB — tablas bbdd_shelies.wa_conversations + bbdd_shelies.wa_messages
...
export async function ensureWaTables(): Promise<void> {
  await query(`
    CREATE TABLE IF NOT EXISTS bbdd_shelies.wa_conversations (
  ...
  `);
  await query(`
    CREATE TABLE IF NOT EXISTS bbdd_shelies.wa_messages (
      id VARCHAR(255) PRIMARY KEY,
      conversation_id VARCHAR(50) NOT NULL REFERENCES bbdd_shelies.wa_conversations(id) ON DELETE CASCADE,
...
export async function getWaConversations(): Promise<WaConversation[]> {
  return query<WaConversation>(
    `SELECT * FROM bbdd_shelies.wa_conversations ORDER BY last_message_at DESC`
  );
}

export async function getWaConversation(id: string): Promise<WaConversation | null> {
  return queryOne<WaConversation>(
    `SELECT * FROM bbdd_shelies.wa_conversations WHERE id = $1`, [id]
  );
}
...
export async function upsertWaConversation(
  ...
  await query(`
    INSERT INTO bbdd_shelies.wa_conversations (id, contact_name, contact_phone, last_message, last_message_at, unread, updated_at)
    VALUES ($1, $2, $3, $4, to_timestamp($5), 1, NOW())
    ON CONFLICT (id) DO UPDATE SET
      contact_name = EXCLUDED.contact_name,
      last_message = EXCLUDED.last_message,
      last_message_at = EXCLUDED.last_message_at,
      unread = bbdd_shelies.wa_conversations.unread + 1,
      updated_at = NOW()
...
export async function addOutboundConversation(
  ...
  await query(`
    INSERT INTO bbdd_shelies.wa_conversations (id, contact_name, contact_phone, last_message, last_message_at, status)
...
export async function markConversationRead(id: string): Promise<void> {
  await query(
    `UPDATE bbdd_shelies.wa_conversations SET unread = 0, updated_at = NOW() WHERE id = $1`, [id]
  );
}
...
export async function updateConversationStatus(
  ...
  if (assignedTo !== undefined) {
    await query(
      `UPDATE bbdd_shelies.wa_conversations SET status = $1, assigned_to = $2, updated_at = NOW() WHERE id = $3`,
      [status, assignedTo, id]
    );
  } else {
    await query(
      `UPDATE bbdd_shelies.wa_conversations SET status = $1, updated_at = NOW() WHERE id = $2`,
      [status, id]
    );
  }
}
...
export async function getWaMessages(conversationId: string): Promise<WaMessage[]> {
  return query<WaMessage>(
    `SELECT * FROM bbdd_shelies.wa_messages WHERE conversation_id = $1 ORDER BY created_at ASC`,
    [conversationId]
  );
}
...
export async function saveWaMessage(msg: Omit<WaMessage, "created_at">): Promise<void> {
  await query(`
    INSERT INTO bbdd_shelies.wa_messages (id, conversation_id, direction, sender_name, text, msg_type, status, wa_timestamp)
